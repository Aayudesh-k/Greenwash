...
export function CommodityBreakdown({ commodities }: CommodityBreakdownProps) {
  ...
  const getTrendIcon = (trend: string) => {
    if (trend === "increasing")
      return <TrendingUp className="w-4 h-4 text-red-600 dark:text-red-500" />;
    if (trend === "decreasing")
      return (
        ...
        <div className="space-y-4">
          {commodities.map((commodity, index) => (
            <div key={index} className="space-y-2">
              ...
              <Progress
                value={commodity.percentOfTotal * 3.3}
                className="h-2"
              />
            </div>
          ))}
          ...
          <div className="flex-1">
            ...
            <p className="text-slate-700 dark:text-slate-300 mb-4">
              ...
              {commodities
                ...
                .reduce((acc, c) => acc + c.percentOfTotal, 0)}
              % of portfolio) are sourced from high or critical deforestation
              risk regions. These commodities are directly linked to areas
              experiencing active forest loss, contradicting sustainability
              commitments.
            </p>
            <Badge
              variant="outline"
              className="text-blue-700 dark:text-blue-400 border-blue-300 dark:border-blue-700 bg-white dark:bg-slate-900"
            >
